...
class QuizViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun startExam(token: String, examId: String = ""){
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val response = dataStore.getDate(examId)
                if(response == null){
                    _uiState.value = UiState.Loading
                    val beginRespose = api.beginExam("Bearer $token", examId).examenes
                    dataStore.saveData(beginRespose.last().examen, beginRespose.last().fecha_hora_inicio)
                    _uiState.value = UiState.Ready
                }
            }catch (e: Exception){
    ...
    fun finishExam(token: String, examId: String = "", calificacion: Int){
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val _response: String? = dataStore.getDate(examId)

                if(_response != null){
                    _uiState.value = UiState.Loading
                    api.finishExam("Bearer $token", examId, EndExamBody(calificacion, _response))

                    dataStore.deleteNamePreferences(examId)
                }
